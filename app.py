...
ALLOWED_EXT = {".mp3", ".wav", ".m4a", ".ogg", ".aac", ".flac", ".wma", ".opus"}
...
def analyze_file(filepath: str, original_name: str, forced_language: str = None) -> dict:
    if forced_language:
        ...
        segments, info = model.transcribe(filepath, beam_size=1, language=forced_language)
...
app.config["MAX_CONTENT_LENGTH"] = 300 * 1024 * 1024  # 300 MB total upload cap

# ---------------------------------------------------------------------------
# Basic auth (important once this app is deployed on the public internet —
# without this, anyone with the URL could upload/view/delete your call data)
# Set APP_USERNAME / APP_PASSWORD as environment variables on your host.
# ---------------------------------------------------------------------------
import secrets
...
def require_login():
    if not _auth_required():
        return  # no creds configured (e.g. local dev) -> auth disabled
    auth = request.authorization
    if not auth or not _check_auth(auth.username, auth.password):
        return Response(
            "Login required", 401, {"WWW-Authenticate": 'Basic realm="Signal"'}
        )
